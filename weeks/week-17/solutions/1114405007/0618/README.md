...
- 速度排名（快到慢）：binary_search > built-in in > set_search > bisect > linear_search
- binary（含先排序成本）大約 n ≈ 5 開始比 linear 快
...
## Stage 3 預測
我的預測binary_search built-in in set_search bisect linear_search
猜測 binary（含先排序成本）大約 n 等於 5 時開始比 linear 快